...
fn parse_yubikey_slot(slot_number: &str) -> Result<Slot, DatabaseKeyError> {
    ...
    Err(DatabaseKeyError::ChallengeResponseKeyError("Invalid slot number".to_string()))
}
    ...
    fn perform_challenge(&self, challenge: &[u8]) -> Result<KeyElement, DatabaseKeyError> {
        match self {
            ChallengeResponseKey::LocalChallenge(secret) => {
                let secret_bytes = hex::decode(secret).map_err(|e| DatabaseKeyError::ChallengeResponseKeyError(e.to_string()))?;
            ...
            ChallengeResponseKey::YubikeyChallenge(yubikey, slot_number) => {
                ...
                let yubikey_device = match challenge_response_client.find_device_from_serial(yubikey.serial_number) {
                    Ok(d) => d,
                    Err(_e) => return Err(DatabaseKeyError::ChallengeResponseKeyError("Yubikey not found".to_string())),
                };
                ...
                match challenge_response_client.challenge_response_hmac(challenge, config) {
                    ...
                    Err(e) => Err(DatabaseKeyError::ChallengeResponseKeyError(format!(
                        "Could not perform challenge response: {e}",
                    ))),
                }
            }
        }
        ...
        let serial_number = match serial_number {
            ...
            None => {
                if all_yubikeys.len() != 1 {
                    return Err(DatabaseKeyError::ChallengeResponseKeyError(
                        "Multiple yubikeys are connected to the system. Please provide a serial number.".to_string(),
                    ));
                }
