...
pub(crate) struct MetaXml {
    ...
    pub memory_protection: Option<MemoryProtectionXml>,
...
impl From<crate::db::Meta> for MetaXml {
    fn from(db: crate::db::Meta) -> Self {
        Self {
            ...
            custom_data: Some(CustomDataXml::db_to_xml(
                &db.custom_data.iter().map(|(k, v)| (k.clone(), v.clone())).collect::<Vec<_>>(),
            )),
        }
    }
}
...
pub struct Binaries {
    ...
    pub binaries: Vec<BinaryXml>,
}

// <Binary ID="..." Compressed="False" Protected="False">BASE64</Binary>
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename = "Binary", rename_all = "PascalCase")]
...
impl BinaryXml {
    pub(crate) fn xml_to_db(self, inner_decryptor: &mut dyn crate::crypt::ciphers::Cipher) -> std::io::Result<crate::db::Value<Vec<u8>>> {
        let mut data = base64_engine::STANDARD.decode(self.value).map_err(std::io::Error::other)?;
        let protected = self.protected.unwrap_or(false);

        if protected {
            data = inner_decryptor.decrypt(&data).map_err(std::io::Error::other)?;
        }
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename = "CustomData", rename_all = "PascalCase")]
pub struct CustomDataXml {
    #[serde(default, rename = "Item")]
    items: Vec<CustomDataItemXml>,
}
            ...
            .map(|item| {
                ...
                (
                    ...
                    crate::db::CustomDataItem {
                        value: if matches!(&value, crate::db::CustomDataValue::Binary(bytes) if bytes.is_empty()) {
                            None
                        } else {
                            Some(value)
                        },
        ...
        let items = db
            ...
            .map(|(key, item)| {
                let value = item
                    ...
                    .as_ref()
                    .map(|v| v.clone().into())
                    .unwrap_or(CustomDataValueXml::String(String::new()));
...
impl From<HashMap<String, crate::db::CustomDataItem>> for CustomDataXml {
...
impl From<CustomDataXml> for HashMap<String, crate::db::CustomDataItem> {
    fn from(cd: CustomDataXml) -> Self {
        cd.items
            .into_iter()
            .map(|item| {
                let value = item.value.into();
...
#[derive(Debug)]
pub enum CustomDataValueXml {
    String(String),
    Binary(Vec<u8>),
}

impl From<CustomDataValueXml> for crate::db::CustomDataValue {
    fn from(val: CustomDataValueXml) -> Self {
        match val {
            CustomDataValueXml::String(s) => crate::db::CustomDataValue::String(s),
            CustomDataValueXml::Binary(b) => crate::db::CustomDataValue::Binary(b),
        }
    }
}

impl From<crate::db::CustomDataValue> for CustomDataValueXml {
    fn from(db: crate::db::CustomDataValue) -> Self {
        match db {
            crate::db::CustomDataValue::String(s) => CustomDataValueXml::String(s),
            crate::db::CustomDataValue::Binary(b) => CustomDataValueXml::Binary(b),
        }
    }
}

impl<'de> Deserialize<'de> for CustomDataValueXml {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match base64_engine::STANDARD.decode(&s) {
            Ok(v) => Ok(CustomDataValueXml::Binary(v)),
            Err(_) => Ok(CustomDataValueXml::String(s)),
        }
    }
}

impl Serialize for CustomDataValueXml {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        match self {
            CustomDataValueXml::String(s) => serializer.serialize_str(s),
            CustomDataValueXml::Binary(b) => {
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename = "MemoryProtection", rename_all = "PascalCase")]
pub(crate) struct MemoryProtectionXml {
...
impl From<MemoryProtectionXml> for crate::db::MemoryProtection {
    fn from(val: MemoryProtectionXml) -> Self {
...
impl From<crate::db::MemoryProtection> for MemoryProtectionXml {
...
mod tests {
    ...
    fn test_serialize_custom_data() {
        let cd = CustomDataXml {
            items: vec![
                CustomDataItemXml {
                    key: "example_key".to_string(),
                    value: CustomDataValueXml::String("example_value".to_string()),
                    last_modification_time: Some(Timestamp::new_base64(NaiveDateTime::from_str("2023-10-05T12:34:56").unwrap())),
                },
                CustomDataItemXml {
                    key: "binary_key".to_string(),
                    value: CustomDataValueXml::Binary(vec![1, 2, 3, 4, 5]),
    ...
    fn test_deserialize_custom_data() {
        ...
        let cd: CustomDataXml = quick_xml::de::from_str(xml).unwrap();
        assert_eq!(cd.items.len(), 2);
        assert_eq!(cd.items[0].key, "example_key");
        match &cd.items[0].value {
            CustomDataValueXml::String(s) => assert_eq!(s, "example_value"),
            _ => panic!("Expected string value"),
        }
        assert_eq!(cd.items[1].key, "binary_key");
        match &cd.items[1].value {
            CustomDataValueXml::Binary(b) => assert_eq!(b, &vec![1, 2, 3, 4, 5]),
    ...
    fn test_serialize_meta() {
        let meta = MetaXml {
            ...
            database_name_changed: Some(Timestamp::new_iso8601(NaiveDateTime::from_str("2023-10-05T12:34:56").unwrap())),
            database_description: Some("A test database".to_string()),
            database_description_changed: Some(Timestamp::new_base64(NaiveDateTime::from_str("2023-10-05T12:34:56").unwrap())),
            default_username: Some("admin".to_string()),
            default_username_changed: Some(Timestamp::new_iso8601(NaiveDateTime::from_str("2023-10-05T12:34:56").unwrap())),
            maintenance_history_days: Some(30),
            color: Some(Color { r: 255, g: 0, b: 0 }),
            master_key_changed: Some(Timestamp::new_base64(NaiveDateTime::from_str("2023-10-05T12:34:56").unwrap())),
            master_key_change_rec: Some(-1),
            master_key_change_force: Some(42),
            memory_protection: Some(MemoryProtectionXml {
                protect_title: Some(true),
                protect_username: Some(false),
                protect_password: Some(true),
                protect_url: Some(false),
            ...
            custom_data: Some(CustomDataXml {
                items: vec![
                    CustomDataItemXml {
                        key: "example_key".to_string(),
                        value: CustomDataValueXml::String("example_value".to_string()),
                    ...
                    CustomDataItemXml {
                        key: "binary_key".to_string(),
                        value: CustomDataValueXml::Binary(vec![1, 2, 3, 4, 5]),
                        last_modification_time: Some(Timestamp::new_iso8601(NaiveDateTime::from_str("2023-10-05T12:34:56").unwrap())),
                    },
                ],
            }),
        };
    ...
    fn test_deserialize_meta() {
        ...
        let meta: MetaXml = quick_xml::de::from_str(xml).unwrap();
        ...
        assert_eq!(binaries.binaries[0].value, base64_engine::STANDARD.encode([1, 2, 3, 4, 5]));
        ...
        assert_eq!(binaries.binaries[1].value, base64_engine::STANDARD.encode([10, 20, 30, 40, 50]));
