...
mod merge_tests {
    ...
    use crate::db::{group_get_children, node_is_group, rc_refcell_node, with_node_mut, Database, Entry, Group, Node, NodePtr, Times};

    fn get_entry(db: &Database, path: &[&str]) -> NodePtr {
        db.root.borrow().as_any().downcast_ref::<Group>().unwrap().get(path).unwrap()
    }

    fn get_group_mut(db: &Database, path: &[&str]) -> NodePtr {
        db.root.borrow().as_any().downcast_ref::<Group>().unwrap().get(path).unwrap()
    }
    ...
    fn test_updated_entry_under_deleted_group() {
        ...
        let modified_entry = destination_db.root.find_entry(&vec![modified_entry_uuid]).unwrap();
    ...
    fn test_entry_relocation_existing_group() {
        ...
        let moved_entry_location = destination_db.root.find_node_location(Uuid::parse_str(ENTRY2_ID).unwrap()).unwrap();
        ...
        assert_eq!(*moved_entry.times.get_location_changed().unwrap(), new_location_changed_timestamp);
    }
    ...
    fn test_entry_relocation_and_update() {
        ...
        assert_eq!(*moved_entry.times.get_last_modification().unwrap(), entry_modified_timestamp,);
        assert_eq!(*moved_entry.times.get_location_changed().unwrap(), new_location_changed_timestamp);
    }
    ...
    fn test_group_relocation() {
        ...
        let mut source_sub_group_1 = match source_group_1.remove_node(&Uuid::parse_str(SUBGROUP1_ID).unwrap()).unwrap() {
            Node::Group(g) => g,
            _ => panic!("This should not happen."),
        };
        thread::sleep(time::Duration::from_secs(1));
        let new_location_changed_timestamp = Times::now();
        source_sub_group_1.times.set_location_changed(new_location_changed_timestamp);
        ...
        let created_entry_location = destination_db.root.find_node_location(Uuid::parse_str(ENTRY2_ID).unwrap()).unwrap();
        assert_eq!(created_entry_location.len(), 3);
